...
export const metadata = { title: "Paragony | Faktura In" };

export default async function InvoicesReceiptsPage() {
  ...
  const receipts = invoices.filter((i) => i.type === "Receipt");
  return (
    <AppShell title="Paragony" subtitle="Paragony fiskalne i uproszczone dokumenty sprzedaży." userEmail={user.email} avatarUrl={profile?.avatarUrl} maxWidth="max-w-[1400px]">
      <Card className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-2xl">Paragony ({receipts.length})</h2>
          <Link href="/invoices/new"><Button className="gap-2"><Plus className="h-4 w-4" />Nowy paragon</Button></Link>
        </div>
        {receipts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="font-display text-xl mb-2">Brak paragonów</h3>
            <p className="text-black/60 dark:text-white/60 max-w-sm">Paragony wystawione w systemie pojawią się tutaj.</p>
          </div>
        ) : (
          <div className="overflow-x-auto rounded-md border border-[#E5E5E5] dark:border-[#262626]">
            <table className="w-full text-left text-sm">
              <thead className="bg-black/5 text-xs font-semibold uppercase tracking-wider dark:bg-white/5">
                <tr><th className="px-4 py-3">Numer</th><th className="px-4 py-3">Data</th><th className="px-4 py-3">Klient</th><th className="px-4 py-3">Kwota</th></tr>
              </thead>
