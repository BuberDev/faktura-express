...
export const metadata = { title: "Faktury proforma | Faktura In" };

export default async function InvoicesProformaPage() {
  ...
  return (
    <AppShell title="Faktury proforma" subtitle="Faktury zaliczkowe i proforma wystawione dla klientów." userEmail={user.email} avatarUrl={profile?.avatarUrl} maxWidth="max-w-[1600px]">
      <Card className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-2xl">Faktury proforma ({proformaInvoices.length})</h2>
          <Link href="/invoices/new"><Button className="gap-2"><Plus className="h-4 w-4" />Nowa proforma</Button></Link>
        </div>
        {proformaInvoices.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gold/10 mb-4">
              <FileText className="h-8 w-8 text-gold-dark dark:text-gold-light" />
            </div>
            <h3 className="font-display text-xl mb-2">Brak faktur proforma</h3>
            <p className="text-black/60 dark:text-white/60 max-w-sm mb-6">Twórz faktury proforma jako wstępne oferty dla klientów.</p>
            <Link href="/invoices/new"><Button>Utwórz proformę</Button></Link>
          </div>
        ) : (
