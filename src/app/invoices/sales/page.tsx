...
export const metadata = { title: "Faktury sprzedaży | Faktura In" };

export default async function InvoicesSalesPage() {
  ...
  // VAT + any type (all outgoing invoices)
  const salesInvoices = invoices.filter((i) => i.type === "VAT" || i.type === "Correction");

  return (
    <AppShell title="Faktury sprzedaży" subtitle="Wszystkie faktury VAT i korekty wystawione dla klientów." userEmail={user.email} avatarUrl={profile?.avatarUrl} maxWidth="max-w-[1600px]">
      <Card className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-2xl">Faktury sprzedaży ({salesInvoices.length})</h2>
          <Link href="/invoices/new"><Button className="gap-2"><Plus className="h-4 w-4" />Wystaw fakturę</Button></Link>
        </div>
        {salesInvoices.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gold/10 mb-4">
              <FilePlus2 className="h-8 w-8 text-gold-dark dark:text-gold-light" />
            </div>
            <h3 className="font-display text-xl mb-2">Brak faktur sprzedaży</h3>
            <p className="text-black/60 dark:text-white/60 max-w-sm mb-6">Wystaw pierwszą fakturę VAT dla swojego klienta.</p>
            <Link href="/invoices/new"><Button>Wystaw fakturę VAT</Button></Link>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-black/5 text-xs font-semibold uppercase tracking-wider dark:bg-white/5">
                <tr>
                  ...
                  <th className="px-4 py-3">KSeF</th>
                  ...
                  <tr key={inv.id} className="hover:bg-black/[0.02] dark:hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-4">
                      <span className={cn("rounded-full px-2 py-1 text-[10px] font-bold uppercase",
                        inv.status === "paid" ? "bg-green-500/10 text-green-600 dark:text-green-400" : "bg-gold/10 text-gold-dark dark:text-gold-light"
                      )}>
                        {inv.status === "paid" ? "Opłacona" : "Nieopłacona"}
                      </span>
                    </td>
                    <td className="px-4 py-4">
                      {inv.ksefStatus && inv.ksefStatus !== "none" ? (
                        <span className="rounded-full bg-green-500/10 px-2 py-0.5 text-[10px] font-bold uppercase text-green-600 dark:text-green-400">
                          {inv.ksefStatus}
                        </span>
