...
export const metadata = { title: "Rachunki | Faktura In" };

export default async function InvoicesBillsPage() {
  ...
  // Bills are non-VAT documents — keep type flexible
  const bills = invoices.filter((i) => i.type === "Bill" || (i.type !== "VAT" && i.type !== "Proforma" && i.type !== "Correction" && i.type !== "Receipt" && i.type !== "Offer"));

  return (
    <AppShell title="Rachunki" subtitle="Rachunki wystawiane przez nievatowców." userEmail={user.email} avatarUrl={profile?.avatarUrl} maxWidth="max-w-[1600px]">
      <Card className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-2xl">Rachunki ({bills.length})</h2>
          <Link href="/invoices/new"><Button className="gap-2"><Plus className="h-4 w-4" />Nowy rachunek</Button></Link>
        </div>
        {bills.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gold/10 mb-4">
              <Receipt className="h-8 w-8 text-gold-dark dark:text-gold-light" />
            </div>
            <h3 className="font-display text-xl mb-2">Brak rachunków</h3>
            <p className="text-black/60 dark:text-white/60 max-w-sm mb-6">Rachunki pojawią się tutaj automatycznie po ich wystawieniu.</p>
          </div>
        ) : (
                  ...
                  <tr key={inv.id} className="hover:bg-black/[0.02] dark:hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-4">
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${inv.status === "paid" ? "bg-green-500/10 text-green-600 dark:text-green-400" : "bg-gold/10 text-gold-dark dark:text-gold-light"}`}>
                        {inv.status === "paid" ? "Opłacony" : "Nieopłacony"}
                      </span>
                    </td>
